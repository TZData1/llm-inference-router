...
def save_results(
    df: pd.DataFrame, results_dir: Path, filename_prefix: str, format: str = "csv"
):
    """
    ...
    if not results_dir.is_dir():
        logger.error(
            f"Results directory does not exist: {results_dir}. Cannot save results."
        )
        return
    ...
    try:
        if format == "csv":
            ...
            logger.error(
                f"Unsupported save format: {format}. Use 'csv', 'parquet', or 'json'."
            )
            return
...
def load_results(results_dir: Path, filename_pattern: str, format: str = "csv"):
    ...
    if not results_dir.is_dir():
        logger.error(
            f"Results directory does not exist: {results_dir}. Cannot load results."
        )
        return None
    ...
    try:
        ...
        if not files:
            logger.warning(
                f"No files found matching pattern '{filename_pattern}' in {results_dir}"
            )
            return None
        ...
        if format == "csv":
            ...
            logger.error(
                f"Unsupported load format: {format}. Use 'csv', 'parquet', or 'json'."
            )
            return None
    except Exception as e:
        logger.error(
            f"Failed to load results from {results_dir} with pattern '{filename_pattern}': {e}"
        )
        return None
...
def save_experiment_metadata(metadata: dict, results_dir: Path, filename_prefix: str):
    ...
    if not results_dir.is_dir():
        logger.error(
            f"Results directory does not exist: {results_dir}. Cannot save metadata."
        )
        return
