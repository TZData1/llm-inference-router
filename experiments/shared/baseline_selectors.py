...
class FixedModelSelector(BaselineSelector):
    ...
    def __init__(self, model_id: str):
        ...
        logger.info(
            f"{self.__class__.__name__} initialized to always select model: {self.model_id}"
        )
...
class RandomModelSelector(BaselineSelector):
    ...
    def __init__(self, model_ids: list, seed: int = None):
        if not model_ids:
            raise ValueError(
                "RandomModelSelector requires a non-empty list of model_ids."
            )
        self.model_ids = model_ids
        self.rng = random.Random(seed)
        logger.info(
            f"RandomModelSelector initialized with {len(self.model_ids)} models and seed={seed}"
        )
