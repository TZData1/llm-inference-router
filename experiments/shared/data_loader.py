...
def load_evaluation_dataset(conn, dataset_names: list, samples_per_dataset: int, random_seed: int):
    ...
    try:
        ...
        all_queries_df = pd.read_sql(query_base, conn, params=params if params else None)
        ...
        for dataset in all_queries_df['dataset'].unique():
            ...
            if len(dataset_queries) > samples_per_dataset:
                dataset_samples = dataset_queries.sample(n=samples_per_dataset, random_state=random_seed)
            else:
        ...
        logger.info(f"Loaded and sampled {len(final_queries_df)} query IDs for evaluation.")
...
def check_data_completeness(conn, required_query_ids: list, required_model_ids: list):
    ...
    if not required_query_ids or not required_model_ids:
         logger.warning("Required query IDs or model IDs are empty. Cannot check completeness.")
    ...
    try:
        ...
        logger.info(f"Fetched {len(all_results_df)} existing pregenerated results from DB.")
        required_df = pd.MultiIndex.from_product(
            [required_query_ids, required_model_ids], 
            names=['query_id', 'model_id']
        ...
        if not all(col in all_results_df.columns for col in key_cols):
             logger.error(f"Fetched results DataFrame is missing key columns: {key_cols}")
             return False, pd.DataFrame(), pd.DataFrame()
        ...
        if not is_complete:
            logger.error(f"MISSING {len(missing_combinations)} out of {total_required} pre-generated results!")
        else:
...
def load_query_features(conn, required_query_ids: list) -> pd.DataFrame:
    ...
    try:
        ...
        if features_df.empty:
            logger.warning("No features found in query_features table for the requested query IDs.")
            return pd.DataFrame()
        features_df = features_df.set_index('query_id')
        missing_feature_ids = set(required_query_ids) - set(features_df.index)
        if missing_feature_ids:
            logger.warning(f"Missing features for {len(missing_feature_ids)} query IDs: {list(missing_feature_ids)[:10]}...")
