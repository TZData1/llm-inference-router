...
def calculate_reward(accuracy_norm: float, energy_norm: float, lambda_weight: float) -> float:
    ...
    reward = (1.0 - lambda_weight) * accuracy_norm + lambda_weight * energy_efficiency_norm
...
def normalize_metrics(inference_results_df: pd.DataFrame):
    ...
    if 'energy_per_token' not in df_copy.columns:
        ...
        if not all(col in df_copy.columns for col in required_cols):
            logger.error(f"Missing columns required for energy_per_token calculation: {required_cols}")
        ...
        for col in required_cols:
            ...
            if corrupted.any():
                logger.warning(f"Corrupted {col} values converted to NaN: {before[corrupted].tolist()}")
    ...
    if 'dataset' not in df_copy.columns:
        logger.error("Input DataFrame must contain a 'dataset' column for normalization.")
        return inference_results_df

    try:
        normalizer = MetricNormalizer()
        if not normalizer.bounds:
            logger.error("Failed to load metric normalization bounds. Check config/metrics.yaml.")
            return inference_results_df
    ...
    for metric in metrics_to_normalize:
        ...
        if metric not in df_copy.columns:
            logger.warning(f"Metric column '{metric}' not found in DataFrame. Skipping normalization.")
...
def calculate_summary_stats(results_df: pd.DataFrame, 
                            groupby_col: str = 'algorithm', 
                            metrics_cols: list = None):
    """
    ...
    if not metrics_cols:
        logger.error("No valid metric columns found or provided for summary statistics.")
        return pd.DataFrame()
    ...
    logger.info(f"Calculating summary statistics grouped by '{groupby_col}' for metrics: {metrics_cols}")
    for col in metrics_cols:
        if col not in results_df.columns:
             logger.warning(f"Metric column '{col}' not found for summary stats. Skipping.")
             continue
        if not pd.api.types.is_numeric_dtype(results_df[col]):
            logger.warning(f"Column '{col}' is not numeric. Attempting conversion for summary stats.")
    ...
    try:
        ...
        if 'model_id' in results_df.columns:
            model_dist = grouped['model_id'].value_counts().unstack(fill_value=0)
            summary[('model_distribution', 'counts')] = grouped['model_id'].apply(lambda x: x.value_counts().to_dict())
...
def find_pareto_frontier(df: pd.DataFrame, 
    ...
    for i, point in df_copy.iterrows():
        ...
        for j, other_point in df_copy.iterrows():
            if i == j: 
                continue
            x_dominates = (other_point[x_col] < point[x_col] if lower_x_is_better else other_point[x_col] > point[x_col])
            y_dominates = (other_point[y_col] > point[y_col] if higher_y_is_better else other_point[y_col] < point[y_col])
...
def prepare_reward_structures(results_norm_df: pd.DataFrame, lambda_weight: float) -> tuple[dict, dict]:
    ...
    if not all(col in results_norm_df.columns for col in required_cols):
        logger.error(f"Input DataFrame missing required columns for reward calculation: {required_cols}")
        return {}, {}
    ...
    try:
        df_copy['reward'] = df_copy.apply(
            lambda row: calculate_reward(row['accuracy_norm'], row['energy_per_token_norm'], lambda_weight),
            axis=1
        )
    except Exception as e:
