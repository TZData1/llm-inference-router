...
def main():
    ...
    try:
        ...
        samples_per_dataset = defaults.get('samples_per_dataset', 500)
         ...
         return
    logger.info(f"Running {EXPERIMENT_NAME} with Features: {feature_names}, Runs per config: {n_runs}, Lambdas: {lambda_values}, Seed: {base_seed}")
    try:
    ...
    if norm_df.empty:
         ...
         if conn: conn.close()
         return
    raw_energy_lkp = raw_results_data_df.set_index(['query_id', 'model_id'])['raw_energy_per_token'].to_dict()
    ...
    try:
        ...
        logger.info(f"Initialized baselines for simulation: {list(baselines_to_run.keys())}")
    except KeyError as e:
    ...
    for baseline_name, selector_instance in baselines_to_run.items():
         if baseline_name in all_strategies:
              logger.warning(f"Baseline name '{baseline_name}' clashes with bandit algorithm name. Bandit config will be used.")
         else:
    ...
    for lambda_weight in lambda_values:
        ...
        for strategy_name, strategy_config in all_strategies.items():
            logger.info(f"--- Running Strategy: {strategy_name} (Lambda={lambda_weight:.2f}) ---")
            ...
            for r in range(n_runs):
                ...
                if strategy_config['type'] == 'bandit':
                    ...
                    if algo_instance is None: 
                        logger.warning(f"Failed to instantiate bandit '{strategy_name}' for run {r+1}, lambda {lambda_weight:.2f}. Skipping run.")
                        continue 
                ...
                for qid_idx, qid in enumerate(tqdm(query_sequence, desc=f"Lambda {lambda_weight:.2f}, Strat '{strategy_name}', Run {r+1}/{n_runs}", leave=False)):
    ...
    if raw_results_data_df is None or 'energy_consumption' not in raw_results_data_df.columns:
        logger.error("Raw results data missing 'energy_consumption'. Cannot calculate total energy per run.")
    else:
        logger.debug("Merging detailed results with raw energy...")
        detailed_with_raw_energy = pd.merge(
            detailed_results_df[['lambda_weight', 'run_id', 'algorithm', 'query_id', 'chosen_model']],
            raw_results_data_df[['query_id', 'model_id', 'energy_consumption']],
            left_on=['query_id', 'chosen_model'],
            right_on=['query_id', 'model_id'],
            how='left'
        )
        logger.debug(f"Merged detailed_with_raw_energy shape: {detailed_with_raw_energy.shape}")
        logger.debug("Grouping by run to sum energy...")
        total_energy_ws_per_run = detailed_with_raw_energy.groupby(
            ['lambda_weight', 'run_id', 'algorithm'], dropna=False
        )['energy_consumption'].sum(min_count=1).reset_index()

        total_energy_ws_per_run['total_energy_wh'] = total_energy_ws_per_run['energy_consumption'] / 3600.0
        run_total_energy_wh = total_energy_ws_per_run[['lambda_weight', 'run_id', 'algorithm', 'total_energy_wh']]
        logger.info(f"Calculated total energy (Wh) for {len(run_total_energy_wh)} run combinations.")
        if not run_total_energy_wh.empty:
            logger.debug(f"Sample of calculated per-run total energy (Wh):\n{run_total_energy_wh.head().to_string()}")
            nan_check = run_total_energy_wh['total_energy_wh'].isnull().sum()
            if nan_check > 0:
                logger.warning(f"Found {nan_check} NaN values in calculated 'total_energy_wh'.")
    ...
    merged_norm_df = pd.merge(
        detailed_results_df[['lambda_weight', 'run_id', 'algorithm', 'query_id', 'chosen_model'] + cols_from_detailed_for_mean],
    ...
    all_metrics_to_average_per_run = metrics_to_average_norm + cols_from_detailed_for_mean
    logger.debug(f"Grouping by run to calculate means for: {all_metrics_to_average_per_run}")
    run_metrics_means = merged_norm_df.groupby(
        ['lambda_weight', 'run_id', 'algorithm'], dropna=False
    )[all_metrics_to_average_per_run].mean().reset_index()
    logger.info(f"Calculated per-run means for normalized metrics and reward for {len(run_metrics_means)} runs.")
    if not run_metrics_means.empty:
         logger.debug(f"Sample of calculated per-run means:\n{run_metrics_means.head().to_string()}")
    if run_total_energy_wh is not None:
        logger.info("Merging per-run total energy (Wh) with other per-run mean metrics...")
        ...
        if 'total_energy_wh' in run_metrics_means.columns:
             ...
             if nan_check_after_merge > 0:
                 logger.warning(f"Found {nan_check_after_merge} NaN values in 'total_energy_wh' after merging into run_metrics_means.")
        else:
        ...
        if 'total_energy_wh' not in run_metrics_means.columns:
            ...
            logger.warning("Total energy (Wh) calculation failed or was skipped, 'total_energy_wh' column added and filled with NaN.")
    ...
    if 'run_id' in run_metrics_means.columns:
        ...
        logger.warning("'run_id' not found in run_metrics_means, cannot calculate count for final summary.")
    ...
    if 'total_energy_wh' in run_metrics_means.columns:
        if run_metrics_means['total_energy_wh'].notna().any():
             ...
             logger.warning("Skipping 'total_energy_wh' in final aggregation as it contains only NaN values.")
    ...
    for col in metrics_for_final_agg:
        if col in run_metrics_means.columns:
            ...
            logger.warning(f"Column '{col}' not found in run_metrics_means during final aggregation setup.")
    if not final_agg_dict or len(grouping_cols) == 0:
        logger.error("No metrics or grouping columns available for final aggregation. Skipping summary stats creation.")
        a6_summary_df = pd.DataFrame()
    else:
        logger.debug(f"Performing final aggregation with rules: {final_agg_dict}")
        try:
            aggregated_means_stddevs = run_metrics_means.groupby(grouping_cols, dropna=False).agg(**final_agg_dict)
            ...
            actual_rename = {k: v for k, v in rename_mapping.items() if k in a6_summary_df.columns}
            if actual_rename:
                 a6_summary_df.rename(columns=actual_rename, inplace=True)
            logger.info(f"Final aggregated summary created with shape {a6_summary_df.shape}.")
            logger.debug(f"Final summary columns: {a6_summary_df.columns.tolist()}")
            if not a6_summary_df.empty:
                 logger.debug(f"Sample of final summary:\n{a6_summary_df.head().to_string()}")
    ...
    try:
        ...
        if 'model_id' not in models_df_plot.columns:
            logger.warning(f"Loaded models file {models_file}, but 'model_id' column is missing.")
            models_df_plot = None
    except FileNotFoundError:
        logger.warning(f"Models file not found at {models_file}. Model sizes will not be annotated.")
    except Exception as e:
        logger.error(f"Error loading models file {models_file}: {e}")
    if not a6_summary_df.empty:
        if 'Mean Total Energy (Wh)' in a6_summary_df.columns and a6_summary_df['Mean Total Energy (Wh)'].notna().any():
            if raw_results_data_df is not None and not raw_results_data_df.empty:
                 ...
                 try:
                     ...
                     logger.error("generate_lambda_sweep_pareto_subplots_wh not found in plotting module.")
                 except Exception as e:
                     logger.error(f"Error generating Total Energy Pareto subplot grid: {e}", exc_info=True)
            else:
                 logger.warning("Skipping Total Energy Pareto plot: Raw results data missing for A1 background.")
        else:
             logger.warning("Skipping Total Energy Pareto plot: 'Mean Total Energy (Wh)' column missing or empty in summary.")
    else:
    ...
    try:
        generate_lambda_sweep_accuracy_total_energy_boxplots_wh(
             run_metrics_combined_df=run_metrics_means,
             exp_dirs=exp_dirs
        )

    except ImportError:
        logger.error("generate_lambda_sweep_accuracy_total_energy_boxplots_wh function not found or import failed.")
    except Exception as e:
        logger.error(f"Error generating combined Accuracy / Total Energy Wh boxplot: {e}", exc_info=True)
