...
class EpsilonGreedy(BaseBandit):
    def __init__(self, model_ids, initial_epsilon=0.1, decay_factor=0.995, 
                 min_epsilon=0.01, seed=None, context_dimension=None):
    ...
    def _get_best_model(self):
        ...
        if avg_rewards:
            ...
            best_models = [model for model, reward in avg_rewards.items() if reward == max_reward]
    ...
    def get_metrics(self):
        metrics = {
            ...
            "exploration_count": sum(1 for d in self.decisions if d["type"] == "exploration"),
            "exploitation_count": sum(1 for d in self.decisions if d["type"] == "exploitation"),
    ...
    def add_model(self, model_id):
        ...
        if model_id not in self.model_ids:
            ...
            logger.info(f"Added model {model_id} to EpsilonGreedy. Total models: {self.n_models}")
        else:
            logger.info(f"Model {model_id} already exists in EpsilonGreedy.")
        

class ContextualEpsilonGreedy(BaseBandit):
    def __init__(self, model_ids, context_dimension=3, initial_epsilon=0.1, 
                 decay_factor=0.995, min_epsilon=0.01, seed=None):
        super().__init__(model_ids, context_dimension)
        self.initial_epsilon = initial_epsilon
        self.epsilon = initial_epsilon
        self.decay_factor = decay_factor
        ...
        if seed is not None:
            random.seed(seed)
            np.random.seed(seed)
        self.context_reward_sums = defaultdict(lambda: {model: 0.0 for model in model_ids})
    ...
    def _get_best_model_for_context(self, context_key):
        if context_key is not None:
            ...
            avg_rewards = {}
            for model_id in self.model_ids:
                avg_rewards[model_id] = self._get_context_average_reward(context_key, model_id)
    ...
    def _get_best_global_model(self):
        for model_id in self.model_ids:
            if self.global_counts[model_id] == 0:
                return model_id
        avg_rewards = {model: self._get_global_average_reward(model) 
                      for model in self.model_ids}
        return max(avg_rewards, key=avg_rewards.get)
        
    def reset(self):
        self.context_reward_sums = defaultdict(lambda: {model: 0.0 for model in self.model_ids})
        self.context_counts = defaultdict(lambda: {model: 0 for model in self.model_ids})
    ...
    def set_state(self, state):
        ...
        self.global_reward_sums = state.get("global_reward_sums", self.global_reward_sums)
    ...
    def get_metrics(self):
        metrics = {
            ...
            "exploration_count": sum(1 for d in self.decisions if d["type"] == "exploration"),
            "exploitation_count": sum(1 for d in self.decisions if d["type"] == "exploitation"),
            "model_selection_counts": {m: self.global_counts[m] for m in self.model_ids},
            "context_count": len(self.context_counts)
        }
        global_avg_rewards = {}
        for model in self.model_ids:
            if self.global_counts[model] > 0:
                global_avg_rewards[model] = self.global_reward_sums[model] / self.global_counts[model]
            else:
    ...
    def add_model(self, model_id):
        ...
        if model_id not in self.model_ids:
            ...
            self.context_reward_sums.default_factory = lambda: {model: 0.0 for model in self.model_ids}
            self.context_counts.default_factory = lambda: {model: 0 for model in self.model_ids}
            ...
            logger.info(f"Added model {model_id} to ContextualEpsilonGreedy. Total models: {self.n_models}")
        else:
    ...
    def get_context_average_rewards(self):
        ...
        for context_key, counts_dict in self.context_counts.items():
            for model_id, count in counts_dict.items():
                if count > 0:
                    total_reward = self.context_reward_sums.get(context_key, {}).get(model_id, 0.0)
...
class LinearEpsilonGreedy(BaseBandit):
    ...
    """
    def __init__(self, model_ids, context_dimension, initial_epsilon, 
                 decay_factor, min_epsilon, lambda_, seed=None):
        """
        Args:
        ...
        if context_dimension is None or context_dimension <= 0:
             raise ValueError("LinearEpsilonGreedy requires a positive context_dimension.")
        ...
        self.A = {mid: self.lambda_ * np.identity(context_dimension) for mid in model_ids}
        ...
        logger.info(f"LinearEpsilonGreedy initialized with d={context_dimension}, lambda={lambda_}")
    ...
    def select_model(self, context):
        ...
        if context is None:
            ...
            logger.warning("LinearEpsilonGreedy received None context, selecting randomly.")
            return random.choice(self.model_ids)
        context = np.asarray(context).reshape(-1)
        if context.shape[0] != self.context_dimension:
             logger.error(f"Context dimension mismatch. Expected {self.context_dimension}, got {context.shape}. Selecting randomly.")
             return random.choice(self.model_ids)
        if random.random() < self.epsilon:
            return random.choice(self.model_ids)
        else:
    ...
    def update(self, model_id, reward, context):
        ...
        if context is None:
            logger.warning(f"Cannot update LinearEpsilonGreedy for arm {model_id} with None context.")
            return

        context = np.asarray(context).reshape(-1)
        if context.shape[0] != self.context_dimension:
             logger.error(f"Cannot update: Context dimension mismatch. Expected {self.context_dimension}, got {context.shape}.")
             return
        ...
        try:
             ...
             logger.error(f"Matrix inversion failed for arm {model_id}. Theta not updated.")
    ...
    def reset(self):
        ...
        self.epsilon = self.initial_epsilon
        self.A = {mid: self.lambda_ * np.identity(self.context_dimension) for mid in self.model_ids}
    ...
    def set_state(self, state):
        ...
        self.A = {mid: np.array(A_list) for mid, A_list in state.get("A", {}).items() if mid in self.model_ids}
        self.b = {mid: np.array(b_list) for mid, b_list in state.get("b", {}).items() if mid in self.model_ids}
        self.theta = {}
        for mid in self.model_ids:
            if mid not in self.A:
                ...
                try:
                    ...
                    logger.error(f"Matrix inversion failed for arm {mid} during set_state. Setting theta to zeros.")
                    self.theta[mid] = np.zeros(self.context_dimension)
    ...
    def add_model(self, model_id):
        ...
        if model_id not in self.model_ids:
            ...
            logger.info(f"Initialized state for new arm {model_id} in LinearEpsilonGreedy.")
