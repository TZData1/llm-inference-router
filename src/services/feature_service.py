...
class FeatureService:
    ...
    def __init__(self, config):
        ...
        self.task_extractor = TaskTypeExtractor(feature_config.get('task_classification', {}))
        self.semantic_extractor = SemanticClusterExtractor(feature_config.get('semantic_clustering', {}))
        self.complexity_extractor = ComplexityExtractor(feature_config.get('complexity', {}))
        print(f"Task types: {self.task_extractor.task_types}; Semantic clusters: {self.semantic_extractor.k}; Complexity bins: {self.complexity_extractor.complexity_bins}")
        ...
        print(f"FeatureService: {len(self.task_types)} tasks, "
    ...
    def extract_features(self, query_text, query_metadata=None):
        ...
        if dataset_name and dataset_name in self.task_types:
             ...
             logger.warning(f"Dataset '{dataset_name}' not in configured task_types {self.task_types} or not provided. Setting task_type to None.")
             features["task_type"] = self.task_extractor.extract(query_text, query_metadata)
        start_time = time.time()
        features["semantic_cluster"] = self.semantic_extractor.extract(query_text, query_metadata)
        start_time = time.time()
        complexity_result = self.complexity_extractor.extract(query_text, query_metadata)
