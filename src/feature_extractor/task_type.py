...
class TaskTypeExtractor:
    ...
    def __init__(self, config=None):
        ...
        self.task_types = self.config.get('task_types', ['mmlu', 'gsm8k', 'hellaswag', 'winogrande', 'cnn_dailymail'])
        ...
        try:
            ...
            if self.embedding_model.get_sentence_embedding_dimension() != self.embedding_dim:
                print(f"Warning: Expected embedding dimension {self.embedding_dim} but model outputs "
        ...
        self.classifier_path = self.config.get('classifier_path', 'models/proper_task_classifier.pkl')
    ...
    def _load_classifier(self):
        ...
        print(f"Creating new task classifier (needs training)")
    ...
    def extract(self, query_text, metadata=None):
        ...
        try:
            embedding = self.embedding_model.encode(instruction, show_progress_bar=False)
    ...
    def _keyword_based_classification(self, query_text):
        ...
        if any(x in query_lower for x in ["summarize", "summary", "summarization"]):
            ...
            return "reasoning"
        elif any(x in query_lower for x in ["if", "imagine", "would", "scenario", "hypothetical"]):
            return "common_sense"
        else:
