...
class PynvmlMeasurement(BasePowerMeasurement):
    ...
    def stop(self):
        ...
        if self.device_handle and self.power_samples:
            ...
            raise RuntimeError(
                "GPU power measurement failed: NVML device handle unavailable and no power samples collected"
            )
...
class ZeusMeasurement(BasePowerMeasurement):
    ...
    def stop(self):
        ...
        if self.start_time is None or self.monitor is None:
            raise RuntimeError(
                "Zeus energy measurement failed: monitor not initialized or start time missing"
            )
