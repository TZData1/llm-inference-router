...
class MetricNormalizer:
    ...
    _DEFAULT_CONFIG_PATH = (
        Path(__file__).resolve().parent.parent.parent / "config" / "metrics.yaml"
    )
    ...
    def __init__(self, config_path=None):
        ...
        if not self.bounds:
            logger.warning(
                f"Metric bounds configuration could not be loaded from {self.config_path}. Normalization may use defaults or fail."
            )
    ...
    def _load_bounds(self):
        ...
        try:
            with open(self.config_path, "r") as f:
                ...
                logger.info(
                    f"Successfully loaded metric bounds from {self.config_path}"
                )
                if isinstance(bounds_data, dict):
                    return bounds_data
                else:
                    logger.error(
                        f"Invalid format in metric bounds file {self.config_path}. Expected a dictionary."
                    )
                    return {}
        except FileNotFoundError:
            logger.error(
                f"Metric bounds configuration file not found: {self.config_path}"
            )
            return {}
        except yaml.YAMLError as e:
            logger.error(
                f"Error parsing metric bounds YAML file {self.config_path}: {e}"
            )
            return {}
        except Exception as e:
            logger.error(
                f"An unexpected error occurred loading metric bounds from {self.config_path}: {e}"
            )
            return {}
    ...
    def normalize(self, metric_name, value, dataset):
        ...
        if value is None or np.isnan(value):
            logger.warning(
                f"Cannot normalize None or NaN value for metric '{metric_name}' on dataset '{dataset}'. Returning 0.5."
            )
            return 0.5
        ...
        if not dataset_bounds:
            logger.warning(
                f"No bounds found for dataset '{dataset}' (and no default) in {self.config_path}. Cannot normalize '{metric_name}'. Returning 0.5."
            )
            return 0.5
        ...
        if (
            not metric_bounds
            or "min" not in metric_bounds
            or "max" not in metric_bounds
        ):
            logger.warning(
                f"Incomplete/missing bounds for metric '{metric_name}' in dataset '{dataset}' config. Cannot normalize. Returning 0.5."
            )
            return 0.5
        ...
        if max_val == min_val:
            ...
            logger.debug(
                f"Min and Max bounds are identical ({min_val}) for metric '{metric_name}' on dataset '{dataset}'. Returning 0.5."
            )
            return 0.5
        try:
            value_f = float(value)
            ...
            logger.warning(
                f"Division by zero during normalization for metric '{metric_name}' on dataset '{dataset}'. Min={min_val}, Max={max_val}. Returning 0.5."
            )
            return 0.5
        except (ValueError, TypeError) as e:
            logger.warning(
                f"Type error during normalization calculation for metric '{metric_name}' on dataset '{dataset}'. Value={value}, Min={min_val}, Max={max_val}. Error: {e}. Returning 0.5."
            )
            return 0.5
